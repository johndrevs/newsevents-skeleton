...
FROM 
  ...
  CUNVMCS.EVENTS_TBL EVE, CUNVMCS.events_ref REF
		where	0=0 and EVE.isactive=1 and REF.eventid=EVE.eventid and REF.siteid=#n# and REF.events_ref_level =2 AND (to_char(enddate, 'yyyy-mm-dd') >= '#dateformat(currentdate,"yyyy-mm-dd")#')
	and to_char(pubdate, 'yyyy-mm-dd') <= to_char(SYSDATE, 'yyyy-mm-dd') and to_char(pulldate, 'yyyy-mm-dd') >= to_char(SYSDATE, 'yyyy-mm-dd') order by startdate<!---<cfabort>---></cfquery>
...
<cfscript>
   ...
   <item>
         <title>#title#</title> 
         <description><![CDATA[#qgetevents.startdate[ctr]#<cfif (isDefined("qgetevents.allday") AND qgetevents.allday EQ 1) or timeformat(qgetevents.startdate[ctr],"h:mm:tt") EQ '1:00:am'>
...
<eventid>#qgetevents.eventid[ctr]#</eventid>
         <author>calendar@luc.edu (University Calendar)</author>
         <pubDate>#date# #time#</pubDate>
         <startDate>#CreateODBCDateTime(qgetevents.startdate[ctr])#</startDate>
         <startDate_formatted>#dateformat(qgetevents.startdate[ctr],"mm/dd/yyyy")#</startDate_formatted>
         <endDate><cfif qgetevents.enddate[ctr] neq ''>#CreateODBCDateTime(qgetevents.enddate[ctr])#<cfelse>#CreateODBCDateTime(qgetevents.startdate[ctr])#</cfif></endDate>
   </item>
   </cfoutput>
</cfloop>
<cfoutput>
