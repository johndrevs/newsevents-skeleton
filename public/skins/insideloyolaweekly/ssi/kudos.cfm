...
where	0=0 and news.isactive=1 and news_ref.newsid=news.newsid and news_ref.siteid=(#variables.kudos_id#) and news_ref.news_ref_level =2 and to_char(pubdate, 'yyyy-mm-dd') <= to_char(#variables.now#, 'yyyy-mm-dd') and to_char(pulldate, 'yyyy-mm-dd') >= to_char(#variables.now#, 'yyyy-mm-dd')	
order by priority
</cfquery>
...
<dd style="margin:0;list-style:none;font-size: 11.5px;padding: 0 0 10px 0">#replace(variables.formatbody, "*",chr(34), "ALL")#</dd></cfoutput></dl>