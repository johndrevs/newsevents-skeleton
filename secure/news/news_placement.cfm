...
FROM CUNVMCS.SITES
WHERE sites.isactive =1 and sites.news=1 and SITEID in (#session.publish#) and SITES.siteid  NOT IN(#variables.used#)
order by site_name
</cfquery>
...
SELECT distinct SITES.SITEID, SITES.SITE_NAME
FROM CUNVMCS.SITES
WHERE sites.isactive =1 and sites.news=1 and SITEID in (#session.post#) and SITES.siteid  NOT IN(#variables.used#)
order by site_name
</cfquery>
...
<cfif getplacement.pubdate GTE #now()# or getplacement.pulldate LTE #now()#><strong>This event is inactive</strong>
	...
	<form action="news_placement_action2.cfm" method="post">
		<cfoutput>
			<p><strong>Publish Article to:</strong>
			<br/>				
			...
			<cfelse>
			<p>You either do not currently have access to publish to any sites or this article is already published  to the sites you do.</p>
			</cfif>
			<p><strong>Post Article to:</strong>
			<br/>				
			...
			<cfelse>
			<p>You either do not currently have access to post to any sites or this article is already posted to the sites you do.</p>
			</cfif>
