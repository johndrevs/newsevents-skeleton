<cfif parameterexists(form.date)>
<cfset now=createodbcdate(form.date)>
<cfelseif parameterexists(url.date)>
<cfset now=createodbcdate(url.date)>
<cfelse>
<cfset now=createodbcdate(now())>
</cfif>
<Cfset nowformat=URLEncodedFormat(variables.now)>
...
where	0=0 and news.isactive=1 and news_ref.newsid=news.newsid and news_ref.siteid=(#form.siteid#) and news_ref.news_ref_level =2 and to_char(pubdate, 'yyyy-mm-dd') <= to_char(#variables.now#, 'yyyy-mm-dd') and to_char(pulldate, 'yyyy-mm-dd') >= to_char(#variables.now#, 'yyyy-mm-dd')	
order by priority
</cfquery>
...
from CUNVMCS.news, CUNVMCS.news_ref
	...
	and to_char(pubdate, 'yyyy-mm-dd') <= to_char(#variables.now#, 'yyyy-mm-dd') and to_char(pulldate, 'yyyy-mm-dd') >= to_char(#variables.now#, 'yyyy-mm-dd')
</cfquery>
	...
	<p><table  border="0" cellspacing="1" cellpadding="4" class="tabulargrey">	
		...
		<cfelse>
			...
			<cfset priority=currentrow>					
				...
				<td>					
				<cfif currentrow gt 1><a href="news_priorityupdate.cfm?siteid=#form.siteid#&newsid=#newsid#&move=up&previousnewsid=#previousnewsid#&priority=#variables.priority#&date=#variables.nowformat#"><img src="/newsevents/images/up.gif" alt="" width="47" height="11" border="0"></a>
				</cfif></td>
				<td>					
				<cfif currentrow lt getnews.recordcount><a href="news_priorityupdate.cfm?siteid=#form.siteid#&newsid=#newsid#&move=down&followingnewsid=#followingnewsid#&priority=#variables.priority#&date=#variables.nowformat#"><img src="/newsevents/images/down.gif" alt="" width="47" height="11" border="0"></a>
				</cfif></td>
</tr>			
...
<p>		There are currently #getnewsmax.recordcount# news items active for this site.
		...
		<cfif (getnewsmax.recordcount-getsiteinfo.maxrows) lte 0>There are  #evaluate((getnewsmax.recordcount-getsiteinfo.maxrows)*-1)# places to be filled.
		<cfelse>There are #evaluate(getnewsmax.recordcount-getsiteinfo.maxrows)# news items on deck.
		</cfif></p>
<p>		
To view the site based on a particular date, please enter it the box below:
<form action="news_site_view.cfm" method="post"><input type="text" name="date"> <input type="submit" class="button"><input type="hidden" name="siteid" value="#form.siteid#"></form></p>
	</cfoutput>
