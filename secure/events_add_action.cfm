	...
	<!---<cfif isdefined("form.post") or isdefined("form.publish")>--->
		...
		<cfquery name="insert_news" datasource="#application.datasource_update#">
			INSERT INTO CUNVMCS.events_tbl(lastmod,isactive,PUBDATE,PULLDATE,STARTDATE,<cfif form.enddate neq 'NULL'>ENDDATE,</cfif>SHORTDESC,LONGDESC,EVENTS_LINK,EVENTS_LINK_OVERRIDE,ISPUBLIC,RSVP,RSVP_PUBLIC,EVENTS_COST,INFO_CLOB,userid,locationid,specificlocation,contact,contactinfo,sponsor,created,status,recurring_status,creator,invited,registration_link)
				...
				<cfif form.enddateno EQ 0>to_date ('#dateformat(enddate, "yyyy/mm/dd")# #timeformat(enddate, "HH:mm:ss")#', 'yyyy/mm/dd hh24:mi:ss'),<cfelse>NULL,</cfif>
				<cfqueryparam value="#form.shortdesc#" cfsqltype="cf_sql_varchar" />, '#form.longdesc#','#form.events_link#','#form.events_link_override#',#form.ispublic#, #form.rsvp#, #form.rsvpp#, '#form.events_cost#', 'x', #form.userid#, #form.locationid#, <cfqueryparam value="#form.specificlocation#" cfsqltype="cf_sql_varchar" />, <cfqueryparam value="#form.contact#" cfsqltype="cf_sql_varchar" />, '#form.contactinfo#', <cfqueryparam value="#form.sponsor#" cfsqltype="cf_sql_varchar" />, SYSDATE, 1,1,#form.userid#,'#form.invited#','#form.registration_link#')
		</cfquery>
...
<!--- Begin Law interest --->       
<cfset calendarlist_law='2358,2359,2360,2361,2362,2363,2364,698,2858'>
<cfset posttolaw=0>
...
<cfquery datasource="#application.datasource_update#" name="insertpost">
					...
					VALUES(#getid.maxID#, 138, SYSDATE,1,#form.userid#)
				</cfquery>
 </cfif>

<!--- End Law interest --->      
		<!---Automatically submit to University Calendar 9/16/04--->
            <cfquery datasource="#application.datasource_update#" name="insertpost">
					INSERT INTO CUNVMCS.events_ref(eventID, siteID, actiondate,EVENTS_REF_level,userid)
					VALUES(#getid.maxID#, 0, SYSDATE,1,#form.userid#)
				</cfquery>
...
<cfelseif cgi.HTTP_REFERER contains 'public'><cfif isdefined("url.skin")><cflocation url="calendar_confirmation.cfm?skin=#url.skin#" addtoken="No"></cfif><cflocation url="calendar_confirmation.cfm" addtoken="No"><cfelse><cflocation url="events_list.cfm" addtoken="No"></cfif></cfoutput>