<p><a href="http://loyolaramblers.cstv.com/calendar/events/" target="_blank">Complete List of 
Athletic Events</a></p><br />




<cfset url.year = "#DatePart('yyyy', variables.now)#">
<cfset url.month = "#DatePart('m', variables.now)#">
<cfset url.day = "#DatePart('d', variables.now)#">
...
<!--- Loop through each day of the week. --->
		<cfloop from="1" to="5" index="LoopDay">
...
order by events_tbl.shortdesc</cfquery>
			...
			<cfif getevent.recordcount GT 0><p>		<cfoutput><strong><a href="http://info.luc.edu/newsevents/public/calendar.cfm?view=d&month=#url.Month#&year=#url.Year#&day=#url.day#">#dateformat(currentdate,"m/d")#</a></cfoutput>:</strong><br />
<ul>
					<cfoutput query="getevent">
		<li><cfif events_link_override eq 1><a href="#events_link#"><cfelse><a href="http://info.luc.edu/newsevents/public/calendar_detail.cfm?eventid=#eventid#&siteid=#variables.upcomingevents_id#&month=#url.month#&year=#url.year#&day=#url.day#"></cfif><cfif shortdesc EQ 'Key Academic Date'><strong>#shortdesc#</strong><cfelse>#shortdesc#</cfif></a><br />
	</li></cfoutput></ul></p></cfif>
